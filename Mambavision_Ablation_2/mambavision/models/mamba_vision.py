...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            if attn_placement == "first_half":
                t_blocks = list(range(0, depths[i] // 2))
            else:
                t_blocks = list(range(depths[i] // 2 + 1, depths[i])) \
                if depths[i] % 2 != 0 \
...
@register_pip_model
@register_model
def mamba_vision_T_firsthalf(pretrained=False, **kwargs):
    """Ablation 2: self-attention in first N/2 layers instead of last N/2."""
    _ = kwargs.pop("model_path", None)
    depths = kwargs.pop("depths", [1, 3, 8, 4])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [8, 8, 14, 7])
    dim = kwargs.pop("dim", 80)
    ...
    model = MambaVision(
        ...
        resolution=resolution,
        drop_path_rate=drop_path_rate,
        attn_placement="first_half",    # ← KEY DIFFERENCE
        **kwargs
    )
