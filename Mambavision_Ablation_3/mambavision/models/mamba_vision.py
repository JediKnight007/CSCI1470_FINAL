...
default_cfgs = {
    ...
    'mamba_vision_L3_512_21k': _cfg(url='https://huggingface.co/nvidia/MambaVision-L3-512-21K/resolve/main/mambavision_L3_21k_740m_512.pth.tar',
                            ...
                            input_size=(3, 512, 512),
                            crop_mode='squash'),   
    'mamba_vision_T_noattn': _cfg(url='', 
                            crop_pct=1.0, 
                            input_size=(3, 224, 224), 
                            crop_mode='center'),                            
}
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            level = MambaVisionLayer(dim=int(dim * 2 ** i),
                                     ...
                                     transformer_blocks=[] if no_attn else (list(range(depths[i]//2+1, depths[i])) if depths[i]%2!=0 else list(range(depths[i]//2, depths[i]))),
                                     )
...
@register_pip_model
@register_model
def mamba_vision_T_noattn(pretrained=False, **kwargs):
    """Ablation 3: pure Mamba — no self-attention blocks anywhere."""
