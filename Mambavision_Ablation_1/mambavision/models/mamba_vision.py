...
default_cfgs = {
    ...
    'mamba_vision_L2_512_21k': _cfg(
        ...
        crop_pct=0.93, input_size=(3, 512, 512), crop_mode='squash'),
    'mamba_vision_L3_256_21k': _cfg(
        ...
        crop_pct=1.0, input_size=(3, 256, 256), crop_mode='center'),
    'mamba_vision_L3_512_21k': _cfg(
        ...
        crop_pct=0.93, input_size=(3, 512, 512), crop_mode='squash'),
    # Ablation variant — no pretrained weights
    'mamba_vision_T_nobypass': _cfg(
        url='', crop_pct=1.0, input_size=(3, 224, 224), crop_mode='center'),
}
...
# ============================================================
# ORIGINAL dual-branch mixer (SSM branch + bypass branch)
# ============================================================
class MambaVisionMixer(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2,
        ...
        # Projects to full d_inner — split into x (SSM) and z (bypass) in forward
        ...
        # out_proj takes concatenated [SSM_out, bypass_out] = full d_inner
    ...
    def forward(self, hidden_states):
        ...
        x, z = xz.chunk(2, dim=1)  # SSM branch and bypass branch
        A = -torch.exp(self.A_log.float())
        x = F.silu(F.conv1d(x, self.conv1d_x.weight, self.conv1d_x.bias,
                             padding='same', groups=self.d_inner // 2))
        z = F.silu(F.conv1d(z, self.conv1d_z.weight, self.conv1d_z.bias,
                             padding='same', groups=self.d_inner // 2))
        ...
        # Concatenate SSM output and bypass output, then project
        out = self.out_proj(torch.cat([y, z], dim=-1))
        return out


# ============================================================
# ABLATION: SSM-only mixer, full channel width to match params
# ============================================================
class MambaVisionMixerNoBypass(nn.Module):
    """
    Ablation 1: removes the symmetric bypass (Z) branch entirely.
    Uses full d_inner width on the SSM branch to keep parameter
    count comparable to the original dual-branch design.
    """
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2,
                 dt_rank="auto", dt_min=0.001, dt_max=0.1,
                 dt_init="random", dt_scale=1.0, dt_init_floor=1e-4,
                 conv_bias=True, bias=False, layer_idx=None,
        ...
        # Full d_inner — no split, no bypass
        self.in_proj = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
        self.x_proj = nn.Linear(
        ...
        # Single conv on full d_inner (no bypass conv)
        self.conv1d_x = nn.Conv1d(
        ...
        # out_proj takes full d_inner (same input size as original's concat)
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, counter, transformer_blocks,
                 ...
                 Mlp_block=Mlp, layer_scale=None,
                 mixer_type="default"):          # ← NEW PARAMETER
        super().__init__()
        self.norm1 = norm_layer(dim)
        if counter in transformer_blocks:
            self.mixer = Attention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias,
                qk_norm=qk_scale, attn_drop=attn_drop,
                proj_drop=drop, norm_layer=norm_layer,
            )
        elif mixer_type == "no_bypass":          # ← ABLATION BRANCH
            self.mixer = MambaVisionMixerNoBypass(
...
class MambaVisionLayer(nn.Module):
    def __init__(self, dim, depth, num_heads, window_size,
                 conv=False, downsample=True, mlp_ratio=4.,
        ...
        if conv:
            ...
            self.blocks = nn.ModuleList([
                Block(dim=dim, counter=i,
                      ...
                      mixer_type=mixer_type)    # ← PASS THROUGH
                for i in range(depth)
            ])
...
class MambaVision(nn.Module):
    def __init__(self, dim, in_dim, depths, window_size, mlp_ratio,
                 num_heads, drop_path_rate=0.2, in_chans=3,
                 num_classes=1000, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., layer_scale=None,
...
# ============================================================
# Model registrations — all at module level, outside classes
# ============================================================
...
def mamba_vision_T(pretrained=False, **kwargs):
    ...
    model = MambaVision(depths=depths, num_heads=num_heads, window_size=window_size,
                        dim=dim, in_dim=in_dim, mlp_ratio=mlp_ratio,
                        resolution=resolution, drop_path_rate=drop_path_rate,
                        mixer_type="default", **kwargs)
    model.pretrained_cfg = pretrained_cfg
    model.default_cfg = model.pretrained_cfg
    if pretrained:
        if not Path(model_path).is_file():
            torch.hub.download_url_to_file(url=model.default_cfg['url'], dst=model_path)
...
@register_pip_model
@register_model
def mamba_vision_T_nobypass(pretrained=False, **kwargs):
    """Ablation 1: MambaVision-T without the symmetric bypass branch."""
    _ = kwargs.pop("model_path", None)  # no pretrained weights for ablation
    depths = kwargs.pop("depths", [1, 3, 8, 4])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [8, 8, 14, 7])
    dim = kwargs.pop("dim", 80)
    ...
    model = MambaVision(depths=depths, num_heads=num_heads, window_size=window_size,
                        dim=dim, in_dim=in_dim, mlp_ratio=mlp_ratio,
                        resolution=resolution, drop_path_rate=drop_path_rate,
                        mixer_type="no_bypass",  # ← KEY DIFFERENCE
                        **kwargs)
...
# All other existing model registrations unchanged below
@register_pip_model
@register_model
def mamba_vision_T2(pretrained=False, **kwargs):
    ...
    model = MambaVision(depths=depths, num_heads=num_heads, window_size=window_size,
                        dim=dim, in_dim=in_dim, mlp_ratio=mlp_ratio,
                        resolution=resolution, drop_path_rate=drop_path_rate, **kwargs)
    model.pretrained_cfg = pretrained_cfg
    model.default_cfg = model.pretrained_cfg
    if pretrained:
        if not Path(model_path).is_file():
...
def mamba_vision_B(pretrained=False, **kwargs):
    ...
    model = MambaVision(depths=depths, num_heads=num_heads, window_size=window_size,
                        dim=dim, in_dim=in_dim, mlp_ratio=mlp_ratio,
                        resolution=resolution, drop_path_rate=drop_path_rate,
                        layer_scale=layer_scale, layer_scale_conv=None, **kwargs)
    model.pretrained_cfg = pretrained_cfg
    model.default_cfg = model.pretrained_cfg
    if pretrained:
        if not Path(model_path).is_file():
